...
app.whenReady().then(async () => {
  ...
  for (const [name, paper, width] of [['a4', 'A4', 190], ['thermal80', 'THERMAL', 74], ['thermal58', 'THERMAL', 52]]) {
    const html = invoiceHtml(data, { paper })
    ...
    assert.equal(invoiceHtml(data, { paper, showLogo: false }).includes('<img class="shop-logo"'), false)
